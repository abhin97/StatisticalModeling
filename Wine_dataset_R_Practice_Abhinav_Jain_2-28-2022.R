...
wine_data <- read.csv("C:\\Abhinav _ NEU BOSTON\\ALY6010 Probability Theory & Intro Statistics\\Module 1 Submission\\Wine_tasting_20220222.csv")
wine_data
wine_new <- wine_data
...
ggplot(data = wine_aly_3) + geom_point(mapping=aes(x=POINTS,y=PRICE),color="Green") +
  geom_smooth(mapping=aes(x=POINTS,y=PRICE),color="Red") +labs(title="Wine Ratings: Price vs. Points", subtitle="Wine ratings and their relationship with the price of wine", caption="Data published by WineEnthusiast")
